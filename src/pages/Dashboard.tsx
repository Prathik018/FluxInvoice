...
export default function Dashboard() {
  ...
  return (
    <main className="relative min-h-screen flex flex-col text-white spotlight-bg px-6 md:px-10 py-10">
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="bg-white/10 backdrop-blur-xl border border-white/20 shadow-xl rounded-2xl p-8 md:p-10"
      >
        <h3 className="text-2xl font-semibold mb-6">Welcome back! </h3>

        <p className="text-white/80 mb-10 max-w-2xl">
          Manage your entire invoicing workflow from a beautifully designed dashboard.
          Create, save, and organize invoices effortlessly with real-time previews and secure local storage.
        </p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {actions.map((action, index) => (
            <motion.div
              ...
              transition={{ delay: index * 0.1 }}
              onClick={action.onClick}
              className="cursor-pointer group p-6 rounded-xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-md hover:shadow-lg transition-all duration-200 hover:-translate-y-1"
            >
              <div className="flex items-center gap-4 mb-3">
